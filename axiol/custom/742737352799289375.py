...
class ChemistryHelp(commands.Cog):
    ...
    async def chem_addmsg(self, ctx, *, msg: str = None):
        if msg is not None:
            ...
            GuildCol = await db.CUSTOM_DATABASE[str(ctx.guild.id)]
            ...
            if data is None:
                ...
                await ctx.send(embed=discord.Embed(
                    description=f"Added the message **{msg}** with response **{response}**",
                    color=var.C_BLUE))
            else:
                trigger = msg.split("|")[0].lstrip(' ').rstrip(' ').lower()
                ...
                await ctx.send(embed=discord.Embed(
                    description=f"Added the message **{trigger}** with response **{response}**",
                    color=var.C_BLUE))
        else:
            await ctx.send(embed=discord.Embed(
                description="🚫 You need to define both the message and it's response",
                color=var.C_RED
            ).add_field(name="Format",
        ...
        if msg is not None:
            ...
            if data is not None:
                if msg.lower() in data.keys():
                    ...
                    await GuildCol.update_one(data,
                                              {"$unset": {msg.lower(): ""}})
                    await ctx.send(
                        f"Successfully removed the message **msg** which was having the response **{res}**")
                else:
    ...
    async def chem_allmsgs(self, ctx):
        ...
        if data is not None:
            ...
            def reactioncheck(reaction, user):
                if str(reaction.emoji) == "◀️" or str(
                        reaction.emoji) == "⬅️" or str(
                        reaction.emoji) == "➡️" or str(reaction.emoji) == "▶️":
        ...
        if msg is not None:
            ...
            if data is None:
                ...
                try:
                    ...
                    await ctx.send(embed=discord.Embed(
                        description=f"Added the message **{msg}** with reaction **{emoji}**",
                        color=var.C_BLUE))
                except:
                    await ctx.send(
                        "Sorry but it seems like either the emoji is invalid or it's a custom emoji from a server where I am not in hence can't use this emoji either :(")
        ...
        if msg is not None:
            ...
            if data is not None:
                ...
                if trigger in data.keys():
                    await GuildCol.update(data,
                                          {"$unset": {trigger.lower(): emoji}})
                    await ctx.send(
    ...
    async def on_message(self, message):
        if not message.guild:
            return
        if message.channel.id in [742848285416357970, 742849666256732170,
                                  844657766794788884,
                                  846840113543905383] and not message.author.bot:

            GuildCol = db.CUSTOM_DATABASE[str(message.guild.id)]
