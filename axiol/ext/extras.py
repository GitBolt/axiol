...
class Extras(commands.Cog):
    ...
    async def ping(self, ctx):
        await ctx.send(f"Pong 🏓! Response time: {round(self.bot.latency * 1000)}ms")
    ...
    async def user(self, ctx, user: disnake.User = None):
        ...
        embed = disnake.Embed(title=f"{user} info", color=var.C_MAIN)
    ...
    async def member(self, ctx, member: disnake.Member = None):
        ...
        embed = disnake.Embed(
            title=f"{member} info", description=status, color=var.C_MAIN
        )
        ...
        embed.add_field(
            name="Joined at", value=member.joined_at.strftime("%B %d, %Y"), inline=False
        )
    ...
    async def stats(self, ctx):
        embed = disnake.Embed(title=f"{ctx.guild.name}", color=var.C_TEAL)
