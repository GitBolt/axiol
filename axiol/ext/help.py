...
async def levelinghelp(ctx: commands.Context):
    prefix = await get_prefix(ctx)
    embed = discord.Embed(title="📊 Leveling", description="Ah yes leveling, MEE6 who?", color=var.C_MAIN
...
async def settingshelp(ctx: commands.Context):
    prefix = await get_prefix(ctx)
    embed = discord.Embed(title="⚙️ Settings", description="Configure my settings and plugins for this server :D", color=var.C_MAIN
...
class View(discord.ui.View):
    ...
    @discord.ui.select(placeholder="Select help", options=HELP_OPTIONS)
    async def callback(self, select: discord.ui.select, interaction: discord.Interaction):
        help_type = interaction.data["values"][0]

        if help_type == "AutoMod":
            await interaction.message.edit(embed=await automodhelp(self.ctx))
        elif help_type == "ChatBot":
            await interaction.message.edit(embed=await chatbothelp(self.ctx))
        elif help_type == "Fun":
            await interaction.message.edit(embed=await funhelp(self.ctx))
        elif help_type == "Giveaway":
            await interaction.message.edit(embed=await giveawayhelp(self.ctx))
        elif help_type == "Karma":
            await interaction.message.edit(embed=await karmahelp(self.ctx))
        elif help_type == "Leveling":
            await interaction.message.edit(embed=await levelinghelp(self.ctx))
        elif help_type == "Moderation":
            await interaction.message.edit(embed=await modhelp(self.ctx))
        elif help_type == "Reaction Roles":
            await interaction.message.edit(embed=await rrhelp(self.ctx))
        elif help_type == "Verification":
            await interaction.message.edit(embed=await verifyhelp(self.ctx))
        elif help_type == "Welcome":
            await interaction.message.edit(embed=await welcomehelp(self.ctx))
        elif help_type == "Extras":
            await interaction.message.edit(embed=await extrahelp(self.ctx))
        else:
            await interaction.message.edit(embed=await settingshelp(self.ctx))
...
class Help(commands.Cog):
    ...
    async def help(self, ctx):
        ...
        embed = discord.Embed(
            title="Here to help!",
            description="Select the respective help option from the menu below.",
            color=var.C_MAIN
        ).add_field(name="Note", value="Help embeds for each plugin can also be viewed using sub commands of help which are followed by the plugin name. Like `help automod`"
        ).set_thumbnail(url="https://cdn.discordapp.com/attachments/843519647055609856/845662999686414336/Logo1.png"
    ...
    async def leveling(self, ctx):
        ...
        if GuildDoc.get("Leveling") == True:
            await ctx.send(embed=levelinghelp(ctx))
        else:
