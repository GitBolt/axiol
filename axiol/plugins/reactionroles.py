...
class ReactionRoles(commands.Cog):
    ...
    async def rr(self, 
        ...
        if botrole.position >= role.position:
            GuildDoc = await db.REACTION_ROLES.find_one({"_id": ctx.guild.id})
            if GuildDoc == None:
                ...
                if check() == True:
                    ...
                    await db.REACTION_ROLES.update_one(GuildDoc, newdata)
                    await msg.add_reaction(emoji)
                    await ctx.send(f"Reaction role for {role} using {emoji} setted up! https://discord.com/channels/{ctx.message.guild.id}/{msg.channel.id}/{msg.id}")
        else:
            await ctx.send(embed=discord.Embed(
    ...
    async def allrr(self, ctx):
        ...
        GuildDoc = await db.REACTION_ROLES.find_one({"_id": Guild.id})
        if GuildDoc is not None and GuildDoc["reaction_roles"] != []:
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        GuildDoc = await db.REACTION_ROLES.find_one({"_id": payload.guild_id})
